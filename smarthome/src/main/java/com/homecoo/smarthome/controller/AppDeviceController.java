...
public class AppDeviceController {
	...
	public String appSyncDeviceInfo(String deviceJson, String deviceSpaceJson,
			String versionJson) {
		System.out.println(deviceJson+""+deviceSpaceJson+""+versionJson);
...
//				.parseArray(deviceJson, DeviceDtoApp.class);
		...
		AppVersion appVersion = JSON.parseObject(versionJson,
				AppVersion.class);
...
//			}
			while (iter.hasNext()) {
				...
				boolean b = userSpaceDeviceService.getUserDeviceSpaceByDeviceno(userDeviceSpace.getDeviceNo());
				if (b) {
	...
	public String appGetDeviceInfo(String phonenum,String gatewayNo) {
		...
		try {
			...
			while (iterator.hasNext()) {
				...
				System.out.println(device.toString()+"==========================================deviceState"+deviceState);
				DeviceDtoApp deviceDtoApp=DeviceUtil.GetDeviceDtoApp(device, deviceState);
