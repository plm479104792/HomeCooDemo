...
public class AppUserController {
	...
	public String appSendRegitserCode(String phonenum) {
		...
		try {
			if (row == 0) {
				...
				SmsCode smsCode = SmsCodeUtil.registerappSmsCode(phonenum,
						idendifyCode);
				int a = smsCodeService.addSmsCode(smsCode);
				logger.debug(this.getClass().getName() + "手机号码 :" + phonenum
						+ ", 随机生成验证码 : " + idendifyCode + "注册验证码");
				if (a != -1) {
					msg = MessageUtils
							.appSendRegitserCodeResp(NeedConstant.ERROR_MESSAGE);
				} else {
					msg = MessageUtils
							.appSendRegitserCodeResp(NeedConstant.SUCCESS_MESSAGE);
				}
			} else {
				msg = MessageUtils.appSendRegitserCodeResp(null);
			}
		} catch (Exception e) {
			e.printStackTrace();
			msg = MessageUtils
	...
	public String appUserRegister(String userJson) throws Exception {
		...
		if (userId == 1) {
			...
			message = MessageUtils.appUserRegisterResp(null);
		} else {
			message = MessageUtils
	...
	public String appUpdatePassword(String phonenum, String newPassword) {
		...
		try {
			...
			e.printStackTrace();
			msg = MessageUtils.appUpdatePasswordResp(null);
		}
		String respjson = JSONObject.toJSONString(msg);
		return respjson;
	}
	...
	public String appLogin(String phonenum, String password) {
		...
		if (phonenum.isEmpty() || password.isEmpty()) {
			...
			if (record != null) {
				...
				message = MessageUtils.appLoginResp(NeedConstant.ERROR_MESSAGE,
						user);
			}
		}
		String messageJson = JSONObject.toJSONString(message);
